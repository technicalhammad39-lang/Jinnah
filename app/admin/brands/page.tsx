    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/50 text-sm mt-1">Manage your partner brands</p>
        </div>
        <button 
                  ...
                  <tr key={brand.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="relative w-12 h-12 rounded-lg overflow-hidden bg-white/5 border border-white/10 flex items-center justify-center shrink-0 p-2">
                          {brand.logo ? (
                        ...
                        <div>
                          <div className="font-bold text-white">{brand.brandName}</div>
                          {brand.website && (
                    ...
                    <td className="px-6 py-4">
                      {brand.featured ? (
                        ...
                        <span className="bg-white/5 text-white/50 px-2.5 py-1 rounded-full text-xs font-semibold">Standard</span>
                      )}
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1917] border border-white/5 rounded-3xl w-full max-w-xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            <div className="sticky top-0 bg-[#1a1917] border-b border-white/5 p-6 flex items-center justify-between z-10">
              <h2 className="text-xl font-bold text-white">{editingId ? "Edit Brand" : "Create Brand"}</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-white/5 rounded-full transition-colors text-white/50 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="space-y-2">
                <label className="text-xs font-bold text-white/50 uppercase tracking-wider pl-1">Brand Name</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-white/50 uppercase tracking-wider pl-1">Brand Logo</label>
                <div className="flex gap-4 items-center">
                  {formData.logo ? (
                    <div className="relative w-24 h-24 rounded-xl overflow-hidden border border-white/10 bg-white group">
                    ...
                    <label className="w-24 h-24 rounded-xl border-2 border-dashed border-white/20 flex flex-col items-center justify-center cursor-pointer hover:border-[#FF6A2A] hover:bg-[#FF6A2A]/5 transition-colors">
                      {uploading ? <Loader2 className="w-6 h-6 animate-spin text-[#FF6A2A]" /> : <UploadCloud className="w-6 h-6 text-white/50" />}
                      <span className="text-[10px] text-white/50 mt-1 uppercase font-bold">Upload</span>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-white/50 uppercase tracking-wider pl-1">Description</label>
                <textarea 
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-white/50 uppercase tracking-wider pl-1">Website URL (Optional)</label>
                <input 
                  type="url" 
              ...
              <div className="border-t border-white/5 pt-6">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input 
                    ...
                    className="w-5 h-5 rounded border-white/10 bg-[#121110] text-[#FF6A2A] focus:ring-[#FF6A2A] focus:ring-offset-[#1a1917]"
                  />
                  <span className="text-sm font-medium text-white/80">Featured Brand</span>
                </label>
              </div>
              ...
              <div className="flex justify-end gap-4 pt-4">
                <button
                  ...
                  className="px-6 py-3 rounded-xl font-bold text-white/70 hover:bg-white/5 transition-colors"
                >
                  Cancel
                </button>
                <button
